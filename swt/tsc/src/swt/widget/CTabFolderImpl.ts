...
export abstract class CTabFolderImpl<T> extends ViewGroupImpl<T>{
	//start - body
	static initialize() {
	...
	@decorate(Type(() => CommandAttr))
	@decorate(Expose({ name: "swtBorderVisible" }))
	swtBorderVisible!:CommandAttr<boolean>| undefined;
	@decorate(Type(() => CommandAttr))
	...
	reset() : T {	
		...
		this.swtBorderVisible = undefined;
...
this.textStyle.setTransformer('textStyle');		return this.thisPointer;
	...
	public setSwtBorderVisible(value : boolean) : T {
		this.resetIfRequired();
		if (this.swtBorderVisible == null || this.swtBorderVisible == undefined) {
			this.swtBorderVisible = new CommandAttr<boolean>();
		}
		
		this.swtBorderVisible.setSetter(true);
		this.swtBorderVisible.setValue(value);
		this.orderSet++;
		this.swtBorderVisible.setOrderSet(this.orderSet);
		return this.thisPointer;
	}
...
export class CTabFolder extends CTabFolderImpl<CTabFolder> implements IWidget{
    getThisPointer(): CTabFolder {
        return this;
    }
