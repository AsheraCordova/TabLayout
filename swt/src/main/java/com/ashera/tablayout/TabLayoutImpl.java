...
private void removeTabByAt(int pos) {
	...
	private void setScrollX(Object objValue) {
    	...
    	if (!isInitialised()) {
        	scrollable.addListener(org.eclipse.swt.SWT.Paint, new org.eclipse.swt.widgets.Listener() {
				@Override
				public void handleEvent(org.eclipse.swt.widgets.Event event) {
					scrollable.setOrigin((int) objValue, scrollable.getOrigin().y);
	        		scrollable.removeListener(org.eclipse.swt.SWT.Paint, this);
				}
        	});
    	} else {
			scrollable.setOrigin((int) objValue, scrollable.getOrigin().y);
